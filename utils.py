...
def calculate_dynamic_volatility_multiplier(current_atr_percent: Decimal, historical_atr_data: List[Decimal]) -> Decimal:
    ...
    Uses configurable parameters from config.py for fine-tuning.
    Returns a multiplier between VOL_MIN_MULTIPLIER and VOL_MAX_MULTIPLIER.
    """
    # Import config inside function to avoid circular imports
    try:
        config_module = sys.modules.get('config_eth')
    except:
    ...
    if not historical_atr_data or len(historical_atr_data) < MIN_HISTORY_PERIODS:
        needed = MIN_HISTORY_PERIODS - len(historical_atr_data) if historical_atr_data else MIN_HISTORY_PERIODS
        logging.info(f"📊 VOLATILITY: Building history ({len(historical_atr_data)}/{MIN_HISTORY_PERIODS}). Need {needed} more cycles. Using default: 1.3x")
        return Decimal('1.3')
    
    # Get config values with safe defaults
    RECENT_WEIGHT = getattr(config_module, 'VOL_RECENT_WEIGHT', Decimal('0.7'))
    MOMENTUM_BOOST_MAX = getattr(config_module, 'VOL_MOMENTUM_BOOST_MAX', Decimal('0.15'))
    MOMENTUM_DAMPEN_MAX = getattr(config_module, 'VOL_MOMENTUM_DAMPEN_MAX', Decimal('0.08'))
    Z_INCREMENT = getattr(config_module, 'VOL_Z_SCORE_INCREMENT', Decimal('0.4'))
    
    # HYBRID MEAN CALCULATION: Blend recent and long-term context
    LONG_TERM_WEIGHT = Decimal('1') - RECENT_WEIGHT
    
    # Recent mean (last 50 periods or all if less)
    recent_lookback = min(50, len(historical_atr_data))
    recent_data = historical_atr_data[-recent_lookback:]
    recent_mean = sum(recent_data) / Decimal(str(len(recent_data)))
    
    # Long-term context
    LONG_TERM_NORMAL = Decimal('0.0065')  # 0.65% as baseline
    ABSOLUTE_FLOOR = Decimal('0.0035')    # 0.35% minimum
    
    # Calculate hybrid mean that adapts to market regime
    hybrid_mean = (recent_mean * RECENT_WEIGHT) + (LONG_TERM_NORMAL * LONG_TERM_WEIGHT)
    effective_mean = max(hybrid_mean, ABSOLUTE_FLOOR)
    
    # Calculate standard deviation on recent data only (more responsive)
    vol_std = (sum((x - recent_mean) ** 2 for x in recent_data) / Decimal(str(len(recent_data)))).sqrt()
    
    # Prevent division by zero and handle extremely low volatility
    if vol_std == Decimal('0'):
        if current_atr_percent < Decimal('0.001'):  # Less than 0.1% movement
            logging.info("📊 VOLATILITY: Market completely stagnant. Using maximum width.")
            return getattr(config_module, 'VOL_MULTIPLIER_DEAD', Decimal('2.3'))
        return getattr(config_module, 'VOL_MULTIPLIER_NORMAL', Decimal('1.2'))
    
    # Calculate z-score using responsive recent mean
    z_score = (current_atr_percent - recent_mean) / vol_std
    
    # Log detailed analysis
    #logging.info(f"📊 GRANULAR VOLATILITY ANALYSIS:")
    #logging.info(f"   Current ATR: {current_atr_percent*100:.4f}%")
    #logging.info(f"   Recent Mean ({len(recent_data)} periods): {recent_mean*100:.4f}%")
    #logging.info(f"   Hybrid Mean: {hybrid_mean*100:.4f}%")
    #logging.info(f"   Effective Mean: {effective_mean*100:.4f}%")
    #logging.info(f"   Recent Std Dev: {vol_std*100:.4f}%")
    #logging.info(f"   Z-Score (vs Recent): {z_score:.3f}")
    #logging.info(f"   Config: Recent Weight={RECENT_WEIGHT}, Z-Increment={Z_INCREMENT}")
    
    # Get multiplier values from config with safe defaults
    MULTIPLIER_HYPER = getattr(config_module, 'VOL_MULTIPLIER_HYPER', Decimal('2.5'))
    MULTIPLIER_VHIGH = getattr(config_module, 'VOL_MULTIPLIER_VHIGH', Decimal('2.2'))
    MULTIPLIER_HIGH = getattr(config_module, 'VOL_MULTIPLIER_HIGH', Decimal('1.9'))
    MULTIPLIER_ELEVATED = getattr(config_module, 'VOL_MULTIPLIER_ELEVATED', Decimal('1.7'))
    MULTIPLIER_MODHIGH = getattr(config_module, 'VOL_MULTIPLIER_MODHIGH', Decimal('1.5'))
    MULTIPLIER_SLIGHT_HIGH = getattr(config_module, 'VOL_MULTIPLIER_SLIGHT_HIGH', Decimal('1.3'))
    MULTIPLIER_NORMAL = getattr(config_module, 'VOL_MULTIPLIER_NORMAL', Decimal('1.2'))
    MULTIPLIER_SLIGHT_LOW = getattr(config_module, 'VOL_MULTIPLIER_SLIGHT_LOW', Decimal('1.4'))
    MULTIPLIER_LOW = getattr(config_module, 'VOL_MULTIPLIER_LOW', Decimal('1.6'))
    MULTIPLIER_VLOW = getattr(config_module, 'VOL_MULTIPLIER_VLOW', Decimal('1.8'))
    MULTIPLIER_ELOW = getattr(config_module, 'VOL_MULTIPLIER_ELOW', Decimal('2.0'))
    MULTIPLIER_DEAD = getattr(config_module, 'VOL_MULTIPLIER_DEAD', Decimal('2.3'))
    
    # Determine multiplier based on z-score using configurable increments
    z_increment = float(Z_INCREMENT)
    
    if z_score > 3.0:
        multiplier = MULTIPLIER_HYPER
        level = "HYPER VOLATILE"
    elif z_score > 2.2:
        multiplier = MULTIPLIER_VHIGH
        level = "VERY HIGH VOL"
    elif z_score > 1.6:
        multiplier = MULTIPLIER_HIGH
        level = "HIGH VOLATILITY"
    elif z_score > 1.1:
        multiplier = MULTIPLIER_ELEVATED
        level = "ELEVATED VOL"
    elif z_score > 0.6:
        multiplier = MULTIPLIER_MODHIGH
        level = "MODERATELY HIGH"
    elif z_score > 0.2:
        multiplier = MULTIPLIER_SLIGHT_HIGH
        level = "SLIGHTLY ABOVE NORM"
    elif z_score > -0.2:
        multiplier = MULTIPLIER_NORMAL
        level = "NORMAL RANGE"
    elif z_score > -0.6:
        multiplier = MULTIPLIER_SLIGHT_LOW
        level = "SLIGHTLY BELOW NORM"
    elif z_score > -1.0:
        multiplier = MULTIPLIER_LOW
        level = "LOW VOLATILITY"
    elif z_score > -1.5:
        multiplier = MULTIPLIER_VLOW
        level = "VERY LOW VOL"
    elif z_score > -2.0:
        multiplier = MULTIPLIER_ELOW
        level = "EXTREMELY LOW"
    else:
        multiplier = MULTIPLIER_DEAD
        level = "DEAD MARKET"

    # Volatility trend detection (rate of change)
    if len(historical_atr_data) >= 10:
        prev_atr = historical_atr_data[-5] if len(historical_atr_data) >= 5 else historical_atr_data[0]
        vol_change = ((current_atr_percent - prev_atr) / prev_atr) if prev_atr > Decimal('0') else Decimal('0')
        
        # Get momentum thresholds from config
        INCREASE_THRESHOLD = getattr(config_module, 'VOL_INCREASE_THRESHOLD', Decimal('0.15'))
        DECREASE_THRESHOLD = getattr(config_module, 'VOL_DECREASE_THRESHOLD', Decimal('0.10'))
        
        # Add momentum-based adjustment
        if vol_change > INCREASE_THRESHOLD:
            momentum_boost = min(MOMENTUM_BOOST_MAX, vol_change * Decimal('0.5'))
            multiplier = multiplier * (Decimal('1') + momentum_boost)
            logging.info(f"📊 VOL MOMENTUM: Rapid increase detected (+{vol_change*100:.1f}%). Boosting multiplier by {momentum_boost*100:.1f}%")
        elif vol_change < -DECREASE_THRESHOLD:
            momentum_dampen = min(MOMENTUM_DAMPEN_MAX, abs(vol_change) * Decimal('0.4'))
            multiplier = multiplier * (Decimal('1') - momentum_dampen)
            logging.info(f"📊 VOL MOMENTUM: Rapid decrease detected ({vol_change*100:.1f}%). Reducing multiplier by {momentum_dampen*100:.1f}%")
    
    # Final bounds check using configurable limits
    MIN_MULTIPLIER = getattr(config_module, 'VOL_MIN_MULTIPLIER', Decimal('1.0'))
    MAX_MULTIPLIER = getattr(config_module, 'VOL_MAX_MULTIPLIER', Decimal('2.5'))
    ...
    logging.info(f"📊 Volatilty: {level}: {current_atr_percent*100:.2f}% (z={z_score:.2f}, x̄={recent_mean*100:.2f}%, σ={vol_std*100:.2f}%) → {multiplier}x")
...
def get_atr_for_grid_gap(market_client: Market, current_price: Decimal, maker_fee: Decimal, user_client: User = None, bot_state: Dict[str, Any] = None) -> Decimal:
    ...
    try:
        ...
        if dynamic_gap < min_profitable_gap:
            logging.critical(f"📊 Grid gap: {final_gap*100:.2f}% (BOOSTED from {dynamic_gap*100:.2f}% | "
                            f"ATR: {atr_percent*100:.2f}% × {dynamic_multiplier:.2f}x | "
                            f"Min profit: {min_profitable_gap*100:.2f}%)")
        else:
            logging.critical(f"📊 Grid gap: {final_gap*100:.2f}% (ATR: {atr_percent*100:.2f}% × {dynamic_multiplier:.2f}x | "
